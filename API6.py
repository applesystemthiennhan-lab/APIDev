...
session.close()
access_token = re.findall(r'access_token=([^"]*)&data_access_expiration_time',
                          res)[0]
print(access_token)
...
if checkgetpage == 'Y' or checkgetpage == 'y':
    ...
    getpage = session.get(
        f"https://graph.facebook.com/me/accounts?access_token={access_token}"
    ).json()
