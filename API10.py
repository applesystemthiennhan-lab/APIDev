...
def Delay(value):
    while not (value <= 1):
        value -= 0.123
        print(
            f'''{trang}[{xanh}APIDev{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [{vang}X    {trang}]''',
            '               ',
            end='\r')
        sleep(0.02)
        print(
...
class Facebook:
    ...
    def info(self):
        try:
            get = self.session.get('https://www.facebook.com/me',
                                   headers=self.headers).url
            url = 'https://www.facebook.com/' + get.split(
                '%2F')[-2] + '/' if 'next=' in get else get
    ...
    def timban(self, text):
        try:
            ...
            response = self.session.post(
                ...
                headers=self.headers,
                data=data).json()
            profile = response["data"]["serpResponse"]["results"]["edges"][0][
                'rendering_strategy']['result_rendering_strategies'][0][
                    'view_model']['profile']
    ...
    def ketban(self, idkb):
        try:
            ...
            response = self.session.post(
                ...
                headers=self.headers,
                data=data).json()
            trangthai = response["data"]["friend_request_send"][
                "friend_requestees"]
            if trangthai and trangthai[0].get(
    ...
    def getidgr(self, noidung):
        try:
            ...
            response = self.session.post(
                ...
                headers=self.headers,
                data=data).json()
            thongtin = response['data']["serpResponse"]["results"]['edges'][0][
    ...
    def reaction(self, id, type):
        try:
            ...
            _get = self.session.post('https://www.facebook.com/api/graphql/',
                                     headers=self.headers,
                                     data=data)
    ...
    def comment(self, id, msg):
        try:
            ...
            cmt = self.session.post('https://www.facebook.com/api/graphql/',
                                    headers=self.headers,
                                    data=data).json()
            if '"feedback_submitted":true' in str(
                    cmt) or 'create_comment' in str(cmt):
                return {'status': 'success', 'trangthai': 'thanhcong'}
            else:
                return {'status': 'error', 'trangthai': 'thatbai'}
        except:
    ...
    def follow(self, id):
        try:
            data = {
                ...
                + str(id) + '","tracking":null,"actor_id":"' + str(self.id) +
...
def addcookie():
    ...
    while True:
        i += 1
        cookie = input(
            f'{thanh}{luc}Nhập Cookie Facebook Số{vang} {i}{trang}: {vang}')
        if cookie == '' and i != 1:
            break
        fb = Facebook(cookie)
        info = fb.info()
...
if server != 'LIVE':
    print(
        f'{thanh}{luc}Trạng Thái Server{trang}: {trang}[{do}{server}{trang}]')
    ...
    if os.path.exists(f'cookiefb-add.json') == False:
        ...
        print(
            f'{thanh}{luc}Nhập {do}[{vang}1{do}] {luc}Sử Dụng Cookie Facebook Đã Lưu'
        )
        print(
            f'{thanh}{luc}Nhập {do}[{vang}2{do}] {luc}Nhập Cookie Facebook Mới'
        )
        thanhngang(65)
    ...
    banner()
    comment = input(
        f"{thanh}{luc}Bật comment dạo {do}({vang}y/n{do}){luc}: {vang}").lower(
        ) == 'y'
    group = input(
        f"{thanh}{luc}Bật tham gia nhóm dạo {do}({vang}y/n{do}){luc}: {vang}"
    ).lower() == 'y'
    reaction = input(
        f"{thanh}{luc}Bật thả cảm xúc dạo {do}({vang}y/n{do}){luc}: {vang}"
    ).lower() == 'y'
    follow = input(
        f"{thanh}{luc}Bật follow dạo {do}({vang}y/n{do}){luc}: {vang}").lower(
        ) == 'y'
    add_friend = input(
        f"{thanh}{luc}Bật gửi lời mời kết bạn dạo {do}({vang}y/n{do}){luc}: {vang}"
    ).lower() == 'y'
    thanhngang(65)
    comments = []
    if comment:
        ...
        while True:
            cmt = input(
                f"{thanh}{luc}Nhập nội dung comment số{vang} {i}{trang}: {vang}"
            ).strip()
            if cmt == "":
                break
            comments.append(cmt)
    ...
    if reaction:
        ...
        print(
            f'{thanh}{luc}Nhập {do}[{vang}1{do}]{luc} Để Chạy Tương Tác Like')
        print(
            f'{thanh}{luc}Nhập {do}[{vang}2{do}]{luc} Để Chạy Tương Tác Love')
        print(
            f'{thanh}{luc}Nhập {do}[{vang}3{do}]{luc} Để Chạy Tương Tác Care')
        print(
            f'{thanh}{luc}Nhập {do}[{vang}4{do}]{luc} Để Chạy Tương Tác Haha')
        print(f'{thanh}{luc}Nhập {do}[{vang}5{do}]{luc} Để Chạy Tương Tác Wow')
        print(f'{thanh}{luc}Nhập {do}[{vang}6{do}]{luc} Để Chạy Tương Tác Sad')
        print(
            f'{thanh}{luc}Nhập {do}[{vang}7{do}]{luc} Để Chạy Tương Tác Angry')
        print(
            f'{thanh}{luc}Có Thể Chọn Nhiều Tương Tác {do}({vang}VD: 1345...{do})'
        )
        thanhngang(65)
        chon = input(
            f'{thanh}{luc}Nhập Số Để Chọn Tương Tác{trang}: {vang}').strip()
        ds_camxuc = {
            "1": "LIKE",
            "2": "LOVE",
            "3": "CARE",
    ...
    while (True):
        try:
            JobbBlock = int(
                input(
                    f'{thanh}{luc}Sau Bao Nhiêu Nhiệm Vụ Chống Block{trang}: {vang}'
                ))
            if JobbBlock <= 1:
    ...
    while (True):
        try:
            DelayBlock = int(
                input(
                    f'{thanh}{luc}Sau {vang}{JobbBlock} {luc}Nhiệm Vụ Nghỉ Bao Nhiêu Giây{trang}: {vang}'
                ))
            break
        except:
            print(f'{do}Vui Lòng Nhập Số')
    while (True):
        try:
            JobBreak = int(
                input(
                    f'{thanh}{luc}Sau Bao Nhiêu Nhiệm Vụ Chuyển Acc{trang}: {vang}'
                ))
            if JobBreak <= 1:
                print(f'{do}Vui Lòng Nhập Lớn Hơn 1')
            break
            ...
            print(f'{do}Vui Lòng Nhập Số')
    runidfb = input(
        f'{thanh}{luc}Bạn Có Muốn Ẩn Id Facebook Không? {do}({vang}y/n{do}){luc}: {vang}'
    )
    thanhngang(65)
    stt = 0
    while True:
        ...
        for cookie in listCookie:
            ...
            if 'success' in info:
                namefb = info['name']
                idfb = str(info['id'])
                idrun = idfb[0] + idfb[1] + idfb[2] + "#" * (
                    int(len(idfb) - 3)) if runidfb.upper() == 'Y' else idfb
            else:
                print(f'{do}Cookie Facebook Die ! Đã Xóa Ra Khỏi List !!!')
                listCookie.remove(cookie)
                break
            print(
                f'{luc}Id Facebook{trang}: {vang}{idrun}{do} | {luc}Tên Tài Khoản{trang}: {vang}{namefb}'
            )
            while True:
                try:
                    ...
                    if nhiemvu == "comment":
                        ...
                        if getpost.get('trangthai') == 'thatbai':
                            JobFail += 1
                            print(
                                f'{trang}[{do}ERROR{trang}] {trang}Không Tìm Thấy Bài Nào',
                                '            ',
                                end="\r")
                        else:
                            ...
                            if cmt.get('trangthai') == 'thanhcong':
                                nextDelay, timejob = True, datetime.now(
                                ).strftime('%H:%M:%S')
                                JobSuccess += 1
                                stt += 1
                                print(
                                    f'{do}| {vang}{stt}{do} | {xanh}{timejob}{do} | {vang}{noidung}{do} | {trang}{idpost}{do}'
                                )
                            else:
                                JobFail += 1
                                print(
                                    f'{trang}[{do}ERROR{trang}] {trang}Không Cmt Được',
                                    '            ',
                                    end="\r")
                    ...
                    if nhiemvu == "group":
                        ...
                        ten = requests.get(
                            'https://dhphuoc.click/vietnamese-name-generator?gioitinh='
                            + gioitinh).json()['data']['name']
                        timgr = fb.getidgr(ten)
                        if timgr.get('trangthai') == 'thatbai':
                            JobFail += 1
                            print(
                                f'{trang}[{do}ERROR{trang}] {trang}Không Tìm Thấy Id Group Nào',
                                '            ',
                                end="\r")
                        else:
                            ...
                            if thamgia.get('trangthai') == 'thanhcong':
                                ...
                                stt += 1
                                print(
                                    f'{do}| {vang}{stt}{do} | {xanh}{timejob}{do} | {vang}GROUP{do} | {trang}{id}{do} | {luc}{name}{do}'
                                )
                            else:
                                JobFail += 1
                                print(
                                    f'{trang}[{do}ERROR{trang}] {trang}Không Tham Gia Được Nhóm',
                                    '            ',
                                    end="\r")
                    ...
                    if nhiemvu == "reaction":
                        ...
                        if getpost.get('trangthai') == 'thatbai':
                            ...
                            if tha.get('trangthai') == 'thanhcong':
                                ...
                                stt += 1
                                print(
                                    f'{do}| {vang}{stt}{do} | {xanh}{timejob}{do} | {vang}{camxuc}{do} | {trang}{idpost}{do}'
                                )
                            else:
                                JobFail += 1
                                print(
                                    f'{trang}[{do}ERROR{trang}] {trang}Không Có Nút Cảm Xúc',
                                    '            ',
                                    end="\r")
                    ...
                    if nhiemvu == "follow":
                        ...
                        if timbb.get('trangthai') == 'thatbai':
                            JobFail += 1
                            print(
                                f'{trang}[{do}ERROR{trang}] {trang}Không Tìm Thấy Id Nào',
                                '            ',
                                end="\r")
                        else:
                            ...
                            if fb.follow(id).get('trangthai') == 'thanhcong':
                                ...
                                stt += 1
                                print(
                                    f'{do}| {vang}{stt}{do} | {xanh}{timejob}{do} | {vang}FOLLOW{do} | {trang}{id}{do} | {luc}{name}{do}'
                                )
                            else:
                                JobFail += 1
                                print(
                                    f'{trang}[{do}ERROR{trang}] {trang}Không Có Nút Theo Dõi',
                                    '            ',
                                    end="\r")
                    ...
                    if nhiemvu == "add_friend":
                        ...
                        if timbb.get('trangthai') == 'thatbai':
                            ...
                            if ketban.get('trangthai') == 'thanhcong':
                                ...
                                stt += 1
                                print(
                                    f'{do}| {vang}{stt}{do} | {xanh}{timejob}{do} | {vang}ADD FRIEND{do} | {trang}{id}{do} | {luc}{name}{do}'
                                )
                            else:
                                JobFail += 1
                                print(
                                    f'{trang}[{do}ERROR{trang}] {trang}Không Có Nút Kết Bạn',
                                    '            ',
                                    end="\r")
                    ...
                    if JobFail >= 20:
                        ...
                        if 'spam' in check:
                            print(
                                f'{do}Tài Khoản {vang}{namefb} {do}Đã Bị Spam')
                            fb.clickDissMiss()
                        elif '282' in check:
                            print(
                                f'{do}Tài Khoản {vang}{namefb} {do}Đã Bị Checkpoint282'
                            )
                            listCookie.remove(cookie)
                            chuyen = True
                            break
                            ...
                            print(
                                f'{do}Tài Khoản {vang}{namefb} {do}Đã Bị Checkpoint956'
                            )
                            listCookie.remove(cookie)
                            chuyen = True
                            break
                            ...
                            print(
                                f'{do}Tài Khoản {vang}{namefb} {do}Đã Bị Out Cookie, Đã Xoá Khỏi List'
                            )
                            listCookie.remove(cookie)
                            chuyen = True
                            break
