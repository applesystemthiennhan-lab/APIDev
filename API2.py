...
# Import lại sau khi đã chắc chắn được cài
import requests
...
def bes4(url):
    ...
    if og_description:
        ...
        print(
            "Không tìm thấy thẻ meta với thuộc tính property='og:description'")


def checkauth(red, blue, green, yellow, cyan, magenta, orange, xanhnhat,
              xduong, pink):
    import cloudscraper
    ...
    while True:
        ...
        headers = {
            ...
            auth,
            't':
            'VFZSWk5VOUVVVEJQUkZGNFRXYzlQUT09',
            'User-Agent':
        ...
        if response.status_code == 200:
            ...
            print(
                f"{red}❌ AUTH sai hoặc bị từ chối (status code: {response.status_code})"
            )
            continue
        ...
        if check.get('status') == 200:
            ...
            nicknames = [
                item['nickname'] for item in check['data']
                if 'nickname' in item
            ]
            print('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
...
def get_id_from_nickname_number(ranmau, check, red, blue, green, yellow, cyan,
                                magenta, orange, xanhnhat, xduong, pink):
    while True:

        user_input = input(
            f'~[+]{random.choice(ranmau)}>{random.choice(ranmau)}>{random.choice(ranmau)}> {green}CHỌN ACC TIKTOK MUỐN CHẠY JOB:{green} '
        )
        try:
            n = int(user_input)
...
def getjob(maxjob, delay, auth, idtiktok, red, blue, green, yellow, cyan,
           magenta, orange, xanhnhat, xduong, pink):
    startmaxjob = 1
    job_success = 0
    hea = {
    ...
    while True:
        while True:
            try:
                a = requests.get(
                    f'https://gateway.golike.net/api/advertising/publishers/tiktok/jobs?account_id={idtiktok}&data=null',
                    headers=hea).json()
                break
            except:
        ...
        try:
            ...
            for k in range(delay, -1, -1):
                ...
                print(
                    f'{green}SUCCESS:{red}[{job_success}/{startmaxjob}]{random.choice(ranmau)}LOADING{random.choice(ranmau)}>>{yellow}NVỤ MỚI SAU{random.choice(ranmau)}>>{random.choice(ranmau)}[{k}s]',
                    end='\r')
                sleep(1)
            ...
            if g['status'] == 200:
                job_success += 1
                print(
                    f'{green}SUCCESS:{red}[{job_success}/{startmaxjob}]{cyan}[{time()}]|{random.choice(ranmau)}SUCCESS|{green}FOLLOW|+{g["data"]["prices"]}'
                )
                startmaxjob += 1
                jobloi = 0
                ...
                if g['status'] == 200:
                    ...
                    requests.post('https://gateway.golike.net/api/report/send',
                                  headers=headers,
                                  json=json_data)
                    ...
                    if startmaxjob == maxjob + 1:
                        ...
                        select = input(
                            f'{red}Lỗi nhiều ,Bạn có muốn đổi nick?(y/n):')
                        if select.lower() == 'n':
                            pass
                        else:
                            nicknames = [
                            ...
                            idtiktok = get_id_from_nickname_number(
                                ranmau, check, red, blue, green, yellow, cyan,
                                magenta, orange, xanhnhat, xduong, pink)
                            jobloi = 0
...
def getjob_follow(maxjob, delay, auth, idtiktok, red, blue, green, yellow,
                  cyan, magenta, orange, xanhnhat, xduong, pink):
    startmaxjob = 1
    job_success = 0
    jobloi = 0
    hea = {
                ...
                if g['status'] == 200:
                    ...
                    while True:
                        try:
                            ...
                            print(f'{red}Đang nhận lại phần thưởng...',
                                  end="\r")
                            sleep(2)
                    if g['status'] == 200:
                        job_success += 1
                        ...
                        if startmaxjob == maxjob + 1:
                            ...
                            if select.lower() == 'n':
                                ...
                                for i, nickname in enumerate(nicknames,
                                    ...
                                    pr(text)
                                idtiktok = get_id_from_nickname_number(
                                    ranmau, check, red, blue, green, yellow,
                                    cyan, magenta, orange, xanhnhat, xduong,
                                    pink)
                                jobloi = 0
...
def creat_key():
    ...
    randoma = ''.join(
        random.choice(characters_to_choose_from) for _ in range(10))
    ...
    note = response['short']
    shortlink = requests.get(
        f"https://3link.co/api?api=e21aeb8a5f25d560d53a6881ae7b74703470f98d&url={note}"
    ).json()
...
ranmau = (red, blue, green, yellow, cyan, magenta, orange, xanhnhat, xduong,
          pink)
...
while True:
    ...
    auth, check = checkauth(red, blue, green, yellow, cyan, magenta, orange,
                            xanhnhat, xduong, pink)
    if not os.path.exists("setting_golike.txt"):
        idtiktok = get_id_from_nickname_number(ranmau, check, red, blue, green,
...
{red}[2]:{green}KHÔNG''')
        ...
        setting = {"loaijob": select_job, "delay": delay, "maxjob": maxjob}
        ...
        sleep(1)
        if select_job == 1:
            getjob_follow(maxjob, delay, auth, idtiktok, red, blue, green,
                          yellow, cyan, magenta, orange, xanhnhat, xduong,
                          pink)
        else:
            getjob(maxjob, delay, auth, idtiktok, red, blue, green, yellow,
                   cyan, magenta, orange, xanhnhat, xduong, pink)
        ...
        select_setting = input(
            f'{green}Bạn có muốn sử dụng setting cũ không?[y/n]{cyan}:')
        if select_setting.lower() == 'n':
            os.remove('setting_golike.txt')
            idtiktok = get_id_from_nickname_number(ranmau, check, red, blue,
                                                   green, yellow, cyan,
                                                   magenta, orange, xanhnhat,
                                                   xduong, pink)
            print(f'''~[+]{red}BẠN CÓ MUỐN LỌC JOB LIKE KHÔNG:
{red}[1]:{green}CÓ
{red}[2]:{green}KHÔNG''')
            ...
            try:
                with open("setting_golike.txt", "r") as file:
                    ...
                    sleep(1)
                    if select_job == 1:
                        getjob_follow(maxjob, delay, auth, idtiktok, red, blue,
                                      green, yellow, cyan, magenta, orange,
                                      xanhnhat, xduong, pink)
                    else:
                        getjob(maxjob, delay, auth, idtiktok, red, blue, green,
                               yellow, cyan, magenta, orange, xanhnhat, xduong,
                               pink)
            except json.JSONDecodeError:
                print(
                    "Dữ liệu không hợp lệ. Vui lòng kiểm tra lại định dạng JSON trong tệp."
                )
