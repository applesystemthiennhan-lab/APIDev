...
def print_colorful_box():
    ...
    for line in text_lines:
        ...
        print('\033[94m' + "║  " + colorful + ' ' * (padding - 2) + "║" +
              reset)
    print('\033[96m' + "╚" + top_bottom + "╝" + reset)
    ...
    def init_params(self):
        ...
        try:
            for url in [
                ...
                match = re.search(r'name="fb_dtsg" value="(.*?)"',
                                  response.text)
                if match:
...
def send_messages_thread(messenger, recipient_id, message_list, delay):
    while True:
        ...
        for message in parts:
            ...
            print(
                f"{status_text} {xanhnhat}Cookie {messenger.user_id}{trang} gửi tới box: {xanh_cyan_dam}{recipient_id}{trang} | Nội dung: {vang}{message}{trang}"
            )
...
def main():
    ...
    if cookie_choice == '1':
        try:
            num_cookies = int(
                input(
                    f"{vang}Nhập số lượng cookie muốn sử dụng: {xanh_cyan_dam}"
                ))
            if num_cookies <= 0:
        ...
        for i in range(num_cookies):
            name = input(
                f"{xanh_cyan_dam}Tên người dùng {i+1}:{xanh_cyan_dam} ").strip(
                )
        ...
        try:
            selected = input(
                f"{vang}Nhập lựa chọn (1 hoặc nhiều số, cách nhau bởi ','): {xanh_cyan_dam}"
            ).strip()
            indexes = [
                int(i) - 1 for i in selected.split(",") if i.strip().isdigit()
            ]
            for i in indexes:
    ...
    for i, cookie in enumerate(cookies, 1):
        try:
            ...
            messengers.append(m)
            print(
                f"{xanh_la}Cookie {i}: OK - User ID: {m.user_id}{xanh_cyan_dam}"
            )
        except Exception as e:
    ...
    try:
        delay = float(
            input(f"{vang}Delay giữa mỗi lần gửi (giây): {xanh_cyan_dam}"))
    except:
        delay = 5

    print(f"\n{xanh_la}Chọn kiểu nội dung tin nhắn:{xanh_cyan_dam}")
    print(
        f"{xanh_cyan_dam}1. Nhây có dấu\n2. Nhây không dấu\n3. Nhây réo tên\n4. Tự nhập nội dung muốn gửi"
    )
    ...
    if choice == '1':
        ...
        custom = input(
            f"{vang}Nhập nội dung (các câu cách nhau bằng dấu ','): {xanh_cyan_dam}"
        ).strip()
        if not custom:
    ...
    for messenger in messengers:
        t = threading.Thread(target=send_messages_thread,
                             args=(messenger, recipient_id, message_list,
                                   delay))
        t.daemon = True
        threads.append(t)
        t.start()
