    ...
    try:
        ...
        if res.status_code == 200 and res.text.strip():
            ...
            if not silent_mode:
                ...
                print(
                    f"[!] Status Code: {res.status_code}, Nội dung: {res.text[:100]}"
                )
            return {}
...
while True:
    ...
    login = safe_get_json(
        f'https://traodoisub.com/api/?fields=profile&access_token={tokenn}')
    if 'success' in login:
...
if (a == '2'):
    file = input(
        f'{Cyan}Nhập tên file cần đọc cookie {Yellow}(mỗi cookie 1 dòng): {Red}'
    )
    file = open(f'{file}')
...
clear_terminal()
checkfl = input(
    f'{Cyan}Có làm nhiệm vụ {Red}follow{Cyan} không (on/off): {Red}')
if (checkfl == 'on'):
    sofl = input(f'{Cyan}Nhập số follow/ 1 acc: {Red}')
    sofl = int(sofl)
    delayfl = input(f'{Cyan}Nhập delay follow: {Red}')
checklike = input(
    f'{Cyan}Có làm nhiệm vụ {Red}like{Cyan} không (on/off): {Red}')
if (checklike == 'on'):
...
if ghj == 'on':
    ...
    tenproxy = input(
        f'{Cyan}Nhập tên file chứa proxy {Yellow}(mỗi proxy 1 dòng sẽ tự random proxy/ 1 acc): {Red}'
    )
...
def apifl1(cookies, idfl, uafake, proxie):
    ...
    headers = {
        ...
        'x-ig-www-claim':
        'hmac.AR1UYU8O8XCMl4jZdv4YxiRUxEIymCA_4stpgFmc092K1Kb2',
        'x-instagram-ajax': '1006309104',
    }
    while True:
        try:
            responsefl = requests.post(
                f'https://i.instagram.com/api/v1/web/friendships/{idfl}/follow/',
                headers=headers,
                proxies=proxies,
                timeout=10).json()
...
def apilike1(cookies, idlike, uafake, link, proxie):
    ...
    if (idlike == 'false'):
        ...
        while True:
            try:
                responselike = requests.post(
                    f'https://www.instagram.com/web/likes/{idlike}/like/',
                    headers=headers,
                    proxies=proxies)
    ...
    while True:
        ...
        for i in range(aaa):
            ...
            if (checkdie == 'no-js'):
                ...
                while True:
                    try:
                        cauhinh = safe_get_json(
                            f'https://traodoisub.com/api/?fields=instagram_run&id={ds_user_id}&access_token={tokenn}'
                        )
                        if 'success' in cauhinh:
                            print(
                                f'{Purple}➤ ACCOUNT NUMER {accthu} {Red}<> {Purple}Cấu Hình ID: {Red}{user} {Purple}Thành Công'
                            )
                            break
                        else:
                            check = check + 1
                            print(
                                f'{Purple}➤ ACCOUNT NUMER {accthu} {Red}<> {Purple}Cấu Hình ID: {Red}{user} {Purple}Thất Bại!'
                            )
                            sleep(10)
                            if (check == 3):
                                break
                    except:
                        print(f'CÓ LỖI XÃY RA!!!0   Vui lòng chờ 5s. ',
                              end='\r')
                        sleep(5)
                        print('                                              ',
                              end='\r')
            if (check == 3):
                ...
                if (checkfl == 'on'):
                    ...
                    while True:
                        ...
                        while True:
                            try:
                                job = safe_get_json(
                                    f'https://traodoisub.com/api/?fields=instagram_follow&access_token={tokenn}'
                                )
                                job = job['data']
                                checkid = job[0]['id']
                                break
                        ...
                        if (len(job) >= 1):
                            print(
                                f'{Yellow}Tìm Thấy {len(job)} Nhiệm Vụ FOLLOW       '
                            )
                            for job in job:
                                x = x + 1
                                demfl = demfl + 1
                                ...
                                giay = hnay.second
                                print(
                                    f'   {Red}[{Yellow}{x}{Red}] [{Yellow}{gio}:{phut}:{giay}{Red}] [{Yellow}FOLLOW{Red}] [{Yellow}{idfl}{Red}] ',
                                    end='')
                                if ghj == 'on':
                                ...
                                for attempt in range(max_retry):
                                    duyet = safe_get_json(
                                        f'https://traodoisub.com/api/coin/?type=INS_FOLLOW_CACHE&id={id}&access_token={tokenn}'
                                    )

                                    if duyet.get('data') and isinstance(
                                            duyet['data'], dict):
                                        ...
                                        if not silent_mode:
                                            print(
                                                f"{Yellow}➤ [Lưu ý] Server đang cache kết quả... thử lại sau vài giây (Lần {attempt+1}/{max_retry})"
                                            )
                                        sleep(3)
                                ...
                                if duyet.get('data') and isinstance(
                                    ...
                                    print(
                                        f"{Green}➤ NHẬN: {nhan} <> ĐỢI DUYỆT: {tong}"
                                    )
                                else:
                                    if not silent_mode:
                                        print(
                                            f"{Red}➤ LỖI: Không thể nhận dữ liệu từ server khi duyệt nhiệm vụ!"
                                        )
                                ...
                                for i in range(int(delayfl), -1, -1):
                                    print(
                                        f'{Red}[C25]{Red}[{White}{str(i)}{Red}]  {Green}SLEEP {Red}[{White}|{Red}] ',
                                        end='\r')
                                    sleep(0.2)
                                    print(
                                        f'{Red}[C25]{Red}[{White}{str(i)}{Red}]  {Green}SLEEP {Red}[{White}|{Red}] ',
                            ...
                            if (jjj == 0 or kkk == sofl):
                                try:
                                    sodu = demfl * 800
                                    print(
                                        f'{Purple}➤ ĐÃ XONG JOB FOLLOW {Red}<> {Purple}NHẬN ĐƯỢC {Red}{sodu} COIN {Red}<> {Purple}ĐỢI DUYỆT {Red}{tong}'
                                    )
                                except:
                                    pass
                                break
                        else:
                            print(f'{Green}Không có nhiệm vụ follow !!',
                                  end='\r')
                            sleep(1)
                            print(
                                '                                              ',
                ...
                if (checklike == 'on'):
                    ...
                    while True:
                        ...
                        while True:
                            try:
                                joblike = safe_get_json(
                                    f'https://traodoisub.com/api/?fields=instagram_like&access_token={tokenn}'
                                )
                        ...
                        if (len(joblike) >= 1):
                            print(
                                f'{Yellow}Tìm Thấy {len(joblike)} Nhiệm Vụ LIKE         '
                            )
                            for joblike in joblike:
                                ...
                                giay = hnay.second
                                print(
                                    f'   {Red}[{Yellow}{x}{Red}] [{Yellow}{gio}:{phut}:{giay}{Red}] [{Yellow}LIKE{Red}] [{Yellow}{idjob}{Red}] ',
                                    end='')
                                if ghj == 'on':
                                    lll = apilike1(Cookie, idjob, uafake, link,
                                                   proxie)
                                else:
                                ...
                                if duyet.get('data') and isinstance(
                                    ...
                                    nhanxu = safe_get_json(
                                        f'https://traodoisub.com/api/coin/?type=INS_LIKE&id={idlike}&access_token={tokenn}'
                                    )
                                    if not silent_mode:
                                        print(
                                            f"{Green}➤ Đã gửi nhận xu: {nhanxu}"
                                        )

                                while 'error' in duyet and not silent_mode:
                                    duyet = safe_get_json(
                                        f'https://traodoisub.com/api/coin/?type=INS_LIKE_CACHE&id={idlike}&access_token={tokenn}'
                                    )
                                    if ('success' in duyet):
                                        break
                                ...
                                for i in range(int(delaylike), -1, -1):
                                    print(
                                        f'{Red}[{Green}H{Yellow}D{Blue}T-{Purple}T{Green}O{Yellow}O{Blue}L{Red}]{Red}[{White}{str(i)}{Red}]  {Green}SLEEP {Red}[{White}|{Red}] ',
                                        end='\r')
                                    sleep(0.2)
                                    print(
                                        f'{Red}[{Green}H{Yellow}D{Blue}T-{Purple}T{Green}O{Yellow}O{Blue}L{Red}]{Red}[{White}{str(i)}{Red}]  {Green}SLEEP {Red}[{White}|{Red}] ',
                            ...
                            if (lll == 0 or ooo == solike):
                                try:
                                    sodu = demlike * 500
                                    print(
                                        f'{Purple}➤ ĐÃ XONG JOB LIKE {Red}<> {Purple}NHẬN ĐƯỢC {Red}{sodu} COIN {Red}<> {Purple}ĐỢI DUYỆT {Red}{tong}'
                                    )
                                except:
                                    pass
                                break
                            ...
                            print(f'{Green}Không có nhiệm vụ like!  ',
                                  end='\r')
                            sleep(1)
                            print(
                                '                                                       ',
